...
class WelcomeEvents(commands.Cog):
    ...
    async def on_member_join(self, user):
        try:
            guild = user.guild
            goodbyetoggle1 = DataBase.db3.find_one({"guild_id": guild.id})['welcometoggle']
            goodbyetoggle2 = DataBase.db3.find_one({"guild_id": guild.id})['welcomeembedtoggle']
            channel1 = DataBase.db3.find_one({"guild_id": guild.id})['channel']
            message1 = DataBase.db3.find_one({"guild_id": guild.id})['message']

            if goodbyetoggle2 == 'Enabled':
                
                channel = DataBase.db3.find_one({"guild_id": guild.id})['channel']
                message = DataBase.db3.find_one({"guild_id": guild.id})['message']
                title = DataBase.db3.find_one({"guild_id": guild.id})['title']
                colorr = DataBase.db3.find_one({"guild_id": guild.id})['color']
                color = int(colorr, 0)
